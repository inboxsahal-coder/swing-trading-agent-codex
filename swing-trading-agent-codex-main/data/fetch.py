...
def fetch_bhavcopy():
    ...
    try:
        ...
        for fp in fallback_files:
            try:
                stale = pd.read_csv(fp)
                if stale is not None and not stale.empty and len(getattr(stale, "columns", [])) > 0:
                    print(f"Bhavcopy fallback: using last available cache {os.path.basename(fp)}")
                    return stale
            except Exception:
                continue
    except Exception:
